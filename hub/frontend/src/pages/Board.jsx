...
function ListView({ filteredTasks, columns, setSelectedTask }) {
  return (
...
export default function Board() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      {view === 'list' ? (
        <ListView filteredTasks={filteredTasks} columns={columns} setSelectedTask={setSelectedTask} />
      ) : (
      ...
      {selectedTask && (
        <Suspense fallback={<LoadingScreen label="Carregando detalhes da tarefa..." />}>
          <TaskDetail
            task={selectedTask}
            board={board}
            columns={columns}
