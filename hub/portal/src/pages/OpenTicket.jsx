  ...
  if (done) {
    return (
      <div style={{ textAlign: 'center', padding: '40px 20px' }}>
        ...
        <h2 style={{ fontSize: 24, fontWeight: 700, marginBottom: 12 }}>Chamado aberto com sucesso!</h2>
        <p style={{ color: '#64748b', marginBottom: 8 }}>
          <strong style={{ color: '#00d4ff' }}>#{done.id}</strong> — Você receberá uma confirmação por email com o link de acompanhamento.
        </p>
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 28, flexWrap: 'wrap' }}>
          <button onClick={() => navigate('/acompanhar/' + done.token)} style={btnPrimary}>
            Acompanhar agora
          </button>
          <button onClick={() => { setDone(null); setForm({ name: '', email: '', title: '', description: '', category: 'suporte', urgency: 'media' }) }} style={btnSecondary}>
            Abrir outro chamado
          </button>
        </div>
      </div>
