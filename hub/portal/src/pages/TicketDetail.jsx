...
export default function TicketDetail() {
  ...
  return (
    <div>
      <Link to="/meus-chamados" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: '#64748b', textDecoration: 'none', fontSize: 14, marginBottom: 20 }}>
      ...
      <div style={{ background: '#161b27', border: '1px solid #1e293b', borderRadius: 12, padding: '20px 24px', marginBottom: 24 }}>
        ...
        {ticket.description && (
          <div style={{ marginTop: 14, background: '#0f1117', borderRadius: 8, padding: '12px 16px', fontSize: 14, color: '#94a3b8', whiteSpace: 'pre-wrap', lineHeight: 1.7 }}>
            {ticket.description}
          </div>
        )}
      </div>

      <h3 style={{ fontSize: 13, color: '#64748b', fontWeight: 600, textTransform: 'uppercase', marginBottom: 14, letterSpacing: 0.5 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 28 }}>
        ...
        {(!ticket.messages || ticket.messages.length === 0) && (
          <p style={{ color: '#475569', fontSize: 14 }}>Ainda sem respostas. Aguarde nossa equipe.</p>
        )}
      </div>

      {closed ? (
        <div style={{ background: '#161b27', border: '1px solid #1e293b', borderRadius: 10, padding: '20px 24px', textAlign: 'center' }}>
          <p style={{ color: '#64748b', fontSize: 14, marginBottom: 16 }}>Chamado encerrado. Abra um novo se precisar de mais ajuda.</p>
          <Link to="/novo" style={{ background: '#00d4ff', border: 'none', borderRadius: 8, color: '#0f1117', fontWeight: 700, textDecoration: 'none', padding: '10px 20px', fontSize: 14, display: 'inline-block' }}>
            Novo chamado
          </Link>
        </div>
      ) : (
        <div style={{ background: '#161b27', border: '1px solid #1e293b', borderRadius: 10, padding: 20 }}>
          <p style={{ fontSize: 13, color: '#64748b', marginBottom: 12, fontWeight: 600 }}>Adicionar informação</p>
          {replyDone && <p style={{ color: '#10b981', fontSize: 13, marginBottom: 10 }}>✓ Enviado! Nossa equipe foi notificada.</p>}
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={sendReply} disabled={sending || (!reply.trim() && !attachment)} style={{
              background: '#00d4ff', border: 'none', borderRadius: 8, color: '#0f1117', fontWeight: 700,
